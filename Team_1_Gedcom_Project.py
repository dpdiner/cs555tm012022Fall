...
def isDateSmallerThanCurrentDate(date):
    ...
    try:
        ...
        for row in rowList:
            ...
            if row[1] == "INDI":
                ...
                if readingBirth:
                    birthDate = getDate(row[2].strip())
                    if (isDateSmallerThanCurrentDate(birthDate) == False):
                        raise Exception("Invalid birth date")
                    newIndiv.birthday = birthDate

                elif readingDeath:
                    deathday = getDate(row[2].strip())
                    if (isDateSmallerThanCurrentDate(birthDate) == False):
                        raise Exception("Invalid death date")
                    newIndiv.deathday = deathday
    ...
    try:
        ...
        for row in rowList:
            ...
            if row[1] == "FAM":
                ...
                if readingMarriage:
                    marriageDate = getDate(row[2].strip())
                    if isDateSmallerThanCurrentDate(marriageDate): raise Exception("Invalid Marriage date")
                    newFam.married = marriageDate
                elif readingDivorce:
                    newFam.isDivorced = True
                    divorceDate = getDate(row[2].strip())
                    if isDateSmallerThanCurrentDate(divorceDate): raise Exception("Invalid Divorce date")
                    newFam.divorced = divorceDate
...
def printOutput():
    ...
    indPT.field_names = ["ID", "NAME", "GENDER", "BIRTHDAY",
                         "AGE", "ALIVE", "DEATH", "CHILD", "SPOUSE"]
    famPT.field_names = ["ID", "MARRIED", "DIVORCED", "HUSBAND ID",
                         "HUSBAND NAME", "WIFE ID", "WIFE NAME", "CHILDREN"]

    for individual in sorted(individuals.keys()):
        ind = individuals[individual]
        indPT.add_row([ind.identifier, ind.name, ind.gender, ind.birthday, ind.age,
                      ind.alive, ind.getDeathday(), ind.getChildFam(), ind.getSpouseFam()])
    for family in sorted(families.keys()):
        fam = families[family]
        famPT.add_row([fam.identifier, fam.married, fam.getIsDivorced(
        ), fam.husbandId, fam.husbandName, fam.wifeId, fam.wifeName, fam.getChildren()])
