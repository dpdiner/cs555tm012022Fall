...
## Last name of all males
def getLastNameByID(individuals):
  inPT = PrettyTable()
  ...
  for individual in sorted(individuals.keys()):
    ...
    if ind.gender == "M":
      ...
      inPT.add_row([ind.identifier, last_name])
  print("Individual males Last Name")
  print(inPT)
...
def upcominganniversary(families):
  ...
  for family in sorted(families.keys()):
    ...
    if fam.getIsDivorced() == "" or fam.getIsDivorced() == "NA":
      ...
      if today < dt2 :
        fpt.add_row([f.identifier, Married])
